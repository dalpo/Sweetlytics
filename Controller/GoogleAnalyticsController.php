...
class GoogleAnalyticsController extends GoogleAnalyticsAppController {
  ...
  public $uses = array('Sweetlytics.GoogleAnalyticsAccount');
  ...
  public function show($tableId = null) {
    ...
    if (!empty($this->params['named']['start-date'])) {
      $params['start-date'] = strftime('%Y-%m-%d', strtotime($this->params['named']['start-date']));
    }

    if (!empty($this->params['named']['end-date'])) {
      $params['end-date'] = strftime('%Y-%m-%d', strtotime($this->params['named']['end-date']));
    }

    if (!empty($this->params['named']['dimensions'])) {
      $params['dimensions'] = explode(',', str_replace(' ', '', $this->params['named']['dimensions']));
    }

    if (!empty($this->params['named']['metrics'])) {
      $params['metrics'] = explode(',', str_replace(' ', '', $this->params['named']['metrics']));
    }

    if (!empty($this->params['named']['sort'])) {
      $params['sort'] = explode(',', str_replace(' ', '', $this->params['named']['sort']));
    }
    ...
    $this->set(compact('account', 'start_date', 'end_date', 'dimensions', 'metrics', 'sort', 'dimensionsArray', 'metricsArray', 'sortArray'));
